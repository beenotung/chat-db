...
export async function up(knex: Knex): Promise<void> {
  {
    // alter column (ws_group.is_parent_group_closed) to be nullable
    ...
    if (!(await knex.schema.hasTable('ws_group'))) {
      await knex.schema.createTable('ws_group', table => {
        ...
        table.boolean('is_parent_group').nullable()
        table.boolean('is_parent_group_closed').nullable()
...
export async function down(knex: Knex): Promise<void> {
  {
    // alter column (ws_group.is_parent_group_closed) to be non-nullable
